...
class HutchinsonParameterPerturber:
    def __init__(
        self,
        model,
        opt,
    ...
    def calc_importance(
        self, dataloader: DataLoader, extra_noise=False, epochs=1
    ) -> Dict[str, torch.Tensor]:
        """
        Adapated from: Avalanche: an End-to-End Library for Continual Learning - https://github.com/ContinualAI/avalanche
        Calculate per-parameter, importance
...
def hassd_tuning(
    ...
    for sweep_i in range(sweeps_n):
        ...
        pdr = HutchinsonParameterPerturber(model, optimizer, device, parameters, adaptive=True,epochs=epochs)
        model = model.eval()
...
def unlearn_func(
    model, method, factor=0.1, device="cuda" if torch.cuda.is_available() else "cpu"
):
